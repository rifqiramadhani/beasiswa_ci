...
class Presences_m extends CI_Model{
	...
	public function get_all($paging=true,$start=0,$limit=10){
		$this->db->select('id_kehadiran,kode_mesin,'.$this->table_name.'.tanggal,jam_masuk,jam_keluar,hadir,id_alasan,'.$this->table_name.'.keterangan,t_hari_libur.tanggal');
		$this->db->from($this->table_name);
		$this->db->join('t_hari_libur','t_hari_libur.tanggal =  '.$this->table_name.'.tanggal');
		$this->db->where($this->table_status,'1');
		$this->db->where($this->table_name.'.tanggal != t_hari_libur.tanggal');
		if($paging==true){
			$this->db->limit($limit,$start);
		}
		return $this->db->get();
	...
	public function get_by_id($id_kehadiran){
		$this->db->select('id_kehadiran,t_kehadiran.kode_mesin,nama,tanggal,jam_masuk,jam_keluar,hadir,t_kehadiran.id_alasan,nama_alasan,keterangan,t_hari_libur.tanggal');
		$this->db->from($this->table_name);
	...
	public function get_by_date($tanggal,$nomor_induk){
		...
		//$this->db->where($this->tanggal->get_hari($tanggal) != 'Minggu');
		$this->db->where($this->table_status,'1');
		return $this->db->get();
	}
	
	// public function get_by_date($tanggal,$nomor_induk){
	// 	$this->db->select('id_kehadiran,t_kehadiran.kode_mesin,nama,t_kehadiran.tanggal,jam_masuk,jam_keluar,hadir,t_kehadiran.id_alasan,t_alasan.nama_alasan,t_kehadiran.keterangan,t_hari_libur.tanggal');
	// 	$this->db->from($this->table_name);
	// 	$this->db->join('t_pegawai','t_pegawai.kode_mesin = '.$this->table_name.'.kode_mesin');
	// 	$this->db->join('t_alasan','t_alasan.id_alasan = '.$this->table_name.'.id_alasan');
	// 	$this->db->join('t_hari_libur','t_hari_libur.tanggal =  '.$this->table_name.'.tanggal');
	// 	$this->db->where($this->table_name.'.kode_mesin = t_pegawai.kode_mesin');
	// 	$this->db->where('t_pegawai.nomor_induk',$nomor_induk);
	// 	$this->db->where($this->table_name.'.tanggal',$tanggal);
	// 	//$this->db->where($this->table_name.'.tanggal != t_hari_libur.tanggal');
	// 	//$this->db->where($this->tanggal->get_hari($tanggal) != 'Minggu');
	// 	$this->db->where($this->table_status,'1');
	// 	return $this->db->get();
	...
	public function get_by_range($startDate, $endDate, $nomorInduk){
		/*$this->db->select('id_kehadiran,t_kehadiran.kode_mesin,nama,tanggal,jam_masuk,jam_keluar,hadir,t_kehadiran.id_alasan,nama_alasan,keterangan');*/
		$this->db->select('t_pegawai.nama,t_kehadiran.id_kehadiran,t_kehadiran.kode_mesin,tanggal,jam_masuk,jam_keluar,hadir,t_alasan.id_alasan,nama_alasan,keterangan');
	...
	public function get_rekap($bulan, $tahun){
		...
		$this->db->select('t_pegawai.nomor_induk,nama,t_kehadiran.kode_mesin,tanggal,jam_masuk,jam_keluar,hadir,t_alasan.id_alasan,nama_alasan,keterangan');
		$this->db->from('t_pegawai');
		$this->db->join('t_kehadiran','t_kehadiran.kode_mesin = t_pegawai.kode_mesin');
		$this->db->join('t_alasan','t_kehadiran.id_alasan = t_alasan.id_alasan');
