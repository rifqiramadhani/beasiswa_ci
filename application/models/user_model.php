...
class User_model extends CI_Model{
	private $table_name = 't_pengguna';
	private $table_pk = 'username';
	private $table_status = 't_pengguna.active';
	...
	// 	$this->db->where('t_peserta.username',$username);
	...
	public function save_pengguna($data_pengguna){ //b
		$this->db->insert($this->table_name,$data_pengguna);
	}

	// public function save_peserta($data_peserta){ //b
	// 	$this->db->insert('t_peserta',$data_peserta);
	// }

	// public function update($username,$data_user){
	// 	$this->db->where('t_peserta.username',$username);
	// 	$this->db->update('t_peserta',$data_user);
	// }
	...
	public function check_login(){ //b
		$username = $this->input->post('username');
		$password = sha1($this->input->post('password'));
		//first checking username
		if($this->get_login($username)->num_rows() > 0){
			//username exist
			$data_user = $this->get_login($username)->row_array();
