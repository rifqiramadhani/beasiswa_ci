...
class User extends CI_Controller{
	...
	public function profil(){
		...
		$this->template->display('user/akun_view',$data);
		// echo "<pre>";
		// print_r($data);
		// die();
		// $data = array();
	...
	public function katasandi(){
		...
		$sandi_terpakai = $data['user']['password'];
		if($_POST){
			$sandi_lama = $this->input->post('password_lama');
			$sandi_baru = $this->input->post('password_baru');
		}
		
		if($this->form_validation->run()==FALSE){
			$data_user = array(
							'password' => $sandi_baru, //ambil terakai
			...
			$this->template->display('user/edit_password_view',$data);
		}
		else{
			$data_user = array(
							'password' => $sandi_baru, //ambil terakai
	...
	public function edit($id){
		...
		if($id){
			...
			if($this->form_validation->run()==FALSE){
				...
				if($count > 0){ //jika data terdaftar
				...
				$data_user = array(
							...
							'id_jam_kerja' => $this->input->post('id_jam_kerja'),
							'tanggal_masuk' => $this->tanggal->tanggal_simpan_db($this->input->post('tanggal_masuk')),
							// 'password' => $data['user']['password'], //ambil terakai
							'updated_user' => $this->session->userdata('user_id'),
							'updated_date' => date('Y-m-d H:i:s'),
						);
	...
	public function login(){//b
		...
		if($this->form_validation->run()==FALSE){
			...
			if($this->user_model->check_login()){
				...
				$data_user = $this->user_model->get_login($this->input->post('username'))->row_array();
				$user_sess = array(
								'username' => $data_user['username'],
								'user_fullname' => $data_user['nama_lengkap'],
								'ijazah' => $data_user['ijazah'],
								'user_type_id' => $data_user['tipe_pengguna'],
								'logged_in' => true
							);
				...
				$this->session->set_flashdata('login_failed','<div class="alert alert-danger">Username dan Password tidak terdaftar.</div>');
				redirect('user/login');
			}
		}
	}
	...
	public function check_username_availabilities(){
		...
		$data = array();
		$count = $this->user_model->get_by_id($username)->num_rows();
