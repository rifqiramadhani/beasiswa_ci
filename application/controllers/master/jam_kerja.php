...
class Jam_kerja extends CI_Controller{
		...
		if(!$id){
			...
			if($this->form_validation->run()==FALSE){
				...
				if($count > 0){
					...
					$this->session->set_flashdata('message_alert','<div class="alert alert-error">ID yang Anda gunakan tidak terdaftar.</div>');
	...
	public function delete($id){
		if(!$id){
			...
			$this->session->set_flashdata('message_alert','<div class="alert alert-success">ID yang Anda gunakan tidak terdaftar.</div>');
